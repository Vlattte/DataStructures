...
{
	Node* newNode = new Node(value, this->next, this); //ñîçäàåì íîâûé óçåë ñî çíà÷åíèåì value
	this->next->previous = newNode;
	this->next = newNode; //ñëåäóþùèé èçìåíèëñÿ, íî ïðåäûäóùèé îñòàëñÿ òàêèì æå
}

void DoubleLinkedList::Node::insertPrevious(const ValueType & value) // [2 new (3) 4]
{
	Node* newNode = new Node(value, this, this->previous); //ñîçäàåì íîâûé óçåë ñî çíà÷åíèåì value
	this->previous->next = newNode; //ïðåäûäóùèé èçìåíèëñÿ, íî ñëåäóþùèé îñòàëñÿ òàêèì æå
...
DoubleLinkedList& DoubleLinkedList::operator=(DoubleLinkedList && moveList) noexcept
{
	if (this == &moveList)
	{
		return *this;	//åñëè ýòî ìû, òî âîçâðàùàåì ñåáÿ æå
	}

	forceNodeDelete(_head); //ðåêóðñèâíî óäàëÿåì this
	...
	return *this; //âîçâðàùàåì ñåáÿ ñêîïèðîâàííîãî
...
DoubleLinkedList::Node* DoubleLinkedList::getNode(const size_t pos) const
{
	if (pos < 0)
	{
		assert(pos < 0); //îøèáêà: ðàçìåð ìåíüøå íóëÿ
	}
	else if (pos >= this->_size)
	{
		assert(pos >= this->_size); //îøèáêà: âûøëè çà ãðàíèöó ñïèñêà
	}
	...
	else //åñëè íå ïîñëåäíèé ýëåìåíò
	{
		Node* bufNode = this->_head; // ñîçäàåì áóôåð äëÿ õðàíåíèÿ ãîëîâíîãî óçëà
		for (int i = 0; i < pos; i++)
		{
			bufNode = bufNode->next; //ïåðåáèðàåì ýëåìåíòû äî íóæíîãî
		}
...
void DoubleLinkedList::insert(const size_t pos, const ValueType & value)
{
	...
	if (pos == 0) //åñëè íóæíî âñòàâèòü â íà÷àëî ñïèñêà, òî pushFront
	{
		pushFront(value);
	}
	else if (pos == _size)
	...
	else //èíà÷å ïåðåáèðàåì óçëû è âñòàâëÿåì íà íóæíóþ ïîçèöèþ // [1 2 (3) 4 5] ---> [1 2 (3) <new> 4 5]
	{
...
{
	// ïåðåçàïèñûâàåì ãîëîâíîé óçåë (ïðåäûäóùèé ïî äåôîëòó nullptr)
	++_size;
	if (_size == 2)
	{
		_tail = _head;
	...
	else
	{
		...
		buf->previous = _head; //ïåðåçàïèñûâàåì ïðåäûäóùèé ñòàðîé ãîëîâå 
	}
}
...
void DoubleLinkedList::remove(const size_t pos)
{
	...
	if (pos == 0)
	{
		//åñëè óäàëÿåì ãîëîâó, òî ïåðåçàïèñûâàåì åå
...
{
	size_t cutSize; //óìåíüøåííûé â äâà ðàçà ðàçìåð
	if ((_size % 2) == 0)
	{
		cutSize = _size / 2;
	}
...
void DoubleLinkedList::forceNodeDelete(Node* node)
{
	if (node == nullptr)
	{
		return; // åñëè êîíåö ñïèñêà 
	}

	// óäàëÿåì âñåõ âïåðåäè, ïîêà íå âñòðåòèì nullptr
